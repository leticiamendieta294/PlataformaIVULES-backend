...
public class UsuariosServiceImpl implements UsuariosService, UserDetailsService{
	...
	public String generarNombreUsuario(Integer personaId) throws IvulesException{
		...
		if( personaId == null ) {
			throw new IvulesException.BusinessException("Persona id es requerido");
		}
		
		Personas persona = personasService.obtener(personaId);
		
		if( persona == null ) {
			throw new IvulesException.BusinessException("No se encontro datos de la persona ");
		}
		
		if( persona.getNombres() == null || persona.getNombres().isEmpty() ) {
			throw new IvulesException.BusinessException("Se necesita nombre de la persona ");
		}
		if( persona.getApellidos() == null || persona.getApellidos().isEmpty() ) {
			throw new IvulesException.BusinessException("Se necesita apellido de la persona");
		}
