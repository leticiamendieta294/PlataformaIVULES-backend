...
public class UsuarioService {
    ...
    public void save(NuevoUsuario nuevoUsuario) throws IvulesException{
    	
    	if(  nuevoUsuario.getUsuario().getFuncionarios() == null 
    			|| nuevoUsuario.getUsuario().getFuncionarios().getFuncionarioId() == null ){
    		throw new IvulesException.BusinessException("Datos del funcionario es requerido ");
    	}
    	
    	if(  Globales.Estados.INACTIVO.equals(nuevoUsuario.getUsuario().getFuncionarios().getEstado()) ){
    		throw new IvulesException.BusinessException("El funcionario se encuenta INACTIVO");
    	}
    	...
    	if( usuarioDb != null ) {
    		throw new IvulesException.BusinessException("El funcionario ya cuenta con un usuario ");
    	}
    	
    	Usuario usuario = nuevoUsuario.getUsuario();
    	
    	if( nuevoUsuario.getUsuario().getPassword() == null) {
    		throw new IvulesException.BusinessException("Contraseña requerida ");
		}
    	
    	String valido = validarPassword(nuevoUsuario.getUsuario().getPassword());
    	
    	if( valido != "" ) {
    		throw new IvulesException.BusinessException(valido);
    	}
    ...
    public void update(NuevoUsuario nuevoUsuario) throws IvulesException{
    	...
    	try {
			...
			if( usuarioDb == null ) {
				throw new IvulesException.BusinessException("No se encontró usuario ");
			}
			...
			throw new IvulesException.InternalServerError("Error al actualizar los datos del usuario: " + e.getMessage());
		}
    }
    ...
    public JwtDto autenticar(LoginUsuario loginUsuario) throws IvulesException {
    	...
    	try {
			...
			throw new IvulesException.AuthenticationError("Contraseña incorrecta");
		}    	
